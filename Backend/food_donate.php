...
$sql="CREATE TABLE IF NOT EXISTS food_donate (
...
if (mysqli_query($conn, $sql)) {
    $sql="INSERT INTO food_donate (donar_name, donar_email, donar_mobile, donar_address, prefered_date, prefered_ghat, count_of_people) VALUES ('$donar_name', '$donar_email', '$donar_mobile', '$donar_address', '$prefered_date', '$prefered_ghat', '$count_of_people')";
    if (mysqli_query($conn, $sql)) {
        $sql="SELECT MAX(id) AS max_id FROM food_donate";
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_assoc($result);
        $max_id = $row['max_id'];
        $donar_id = "FOOD" . str_pad($max_id, 10, '0', STR_PAD_LEFT);
        $sql="UPDATE food_donate SET donar_id='$donar_id' WHERE id=$max_id";
        if (mysqli_query($conn, $sql)) {
            echo "Food donation request submitted successfully!";
        } else {
